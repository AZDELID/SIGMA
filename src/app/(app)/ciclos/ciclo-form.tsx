...
export function CicloForm() {
  ...
  return (
    <form
      ref={formRef}
      action={formAction}
      className="grid grid-cols-1 gap-3 rounded-lg border border-sky-200 bg-white p-4 sm:grid-cols-2 md:grid-cols-4"
    >
      ...
      {state.error && (
        <p className="text-sm text-red-600 md:col-span-4">{state.error}</p>
      )}
    </form>
  );
}
